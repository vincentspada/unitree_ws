...
TeleForceCmd::TeleForceCmd()
{
    ...
    Fy = 0;
    Fz = 0;
    node = rclcpp::Node::make_shared("external_force");
    force_pub = node->create_publisher<geometry_msgs::msg::Wrench>("/apply_force/trunk", 20);
...
void TeleForceCmd::keyLoop()
{
    ...
    for(;;){
        ...
        RCLCPP_DEBUG(node->get_logger(), "value: 0x%02X\n", c);
        switch(c){
        case KEYCODE_UP:
            if(mode > 0) {
                Fx = 60;
        ...
        case KEYCODE_SPACE:
            mode = mode*(-1);
            if(mode > 0){
                RCLCPP_INFO(node->get_logger(), "Change to Pulsed mode.");
            } else {
                RCLCPP_INFO(node->get_logger(), "Change to Continuous mode.");
            }
            Fx = 0;
            Fy = 0;
            Fz = 0;            
